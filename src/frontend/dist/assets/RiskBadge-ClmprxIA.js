import { r as reactExports, d as useComposedRefs, j as jsxRuntimeExports, y as vt, o, a as cn } from "./index-Ce0Yvw1c.js";
import { e as useCallbackRef$1, P as Primitive, b as composeEventHandlers, j as dispatchDiscreteCustomEvent, k as useLayoutEffect2, d as useControllableState, u as useId, f as Presence, l as createContext2, c as createContextScope, a as createSlot } from "./use-backend-h-MyA2la.js";
...
function getConditionDisplayName(conditionKey) {
...
const NEXT_STEPS = {
  ...
  moderate: "Monitor for any worsening symptoms. Consider consulting an eye care professional within the next few weeks.",
  elevated: "Schedule an appointment with an ophthalmologist soon. Avoid self-diagnosing and seek professional evaluation.",
...
const RESEARCH_DISCLAIMER = "OcuScreen+ is a research tool and not a medical diagnostic device.";
const CONDITION_MEANINGS = {
  jaundice: "Yellowing of the sclera (whites of the eye) may suggest elevated bilirubin, which can indicate liver, gallbladder, or blood conditions.",
  anemia: "Paleness of the conjunctiva (inner eyelid lining) may indicate reduced haemoglobin levels, commonly associated with nutritional deficiencies or chronic illness.",
  cornealArcus: "A whitish or greyish ring forming around the cornea edge may indicate lipid deposits, sometimes associated with elevated cholesterol, particularly in younger individuals.",
  eyeRedness: "Visible redness of the sclera or conjunctiva may indicate infection, allergy, inflammation, or dryness. Often benign but persistent redness warrants evaluation.",
  pupilIrregularity: "Asymmetry or irregular pupil shape can, in rare cases, reflect neurological or structural eye conditions. Should be evaluated if persistent.",
  drynessIndicator: "Surface dryness signs may indicate reduced tear production or meibomian gland dysfunction, often linked to dehydration, screen exposure, or systemic conditions.",
  dryness: "Surface dryness signs may indicate reduced tear production or meibomian gland dysfunction, often linked to dehydration, screen exposure, or systemic conditions."
};
function getConditionMeaning(conditionKey) {
  return CONDITION_MEANINGS[conditionKey] ?? "This indicator was evaluated as part of the multi-feature screening analysis.";
}
function getEvidenceStrength(featureConfidences) {
  const values = [
  ...
  const above50 = values.filter((v2) => v2 >= 50).length;
  const above70 = values.filter((v2) => v2 >= 70).length;
  if (above70 >= 3) return "Strong";
  if (above50 >= 2) return "Moderate";
  return "Weak";
}
function getConfidenceColorClass(value) {
  if (value >= 70) return "bg-accent";
  if (value >= 40) return "bg-secondary";
  return "bg-destructive";
}
function getConfidenceTextClass(value) {
  if (value >= 70) return "text-accent";
  if (value >= 40) return "text-secondary";
  return "text-destructive";
}
function getTopConditionsForSummary(conditions) {
  return [...conditions].sort((a, b) => b.conditionConfidence - a.conditionConfidence).slice(0, 4);
}
function getOverallImpression(conditions) {
  ...
  if (names.length === 0) {
    const modNames = moderate.map((c) => getConditionDisplayName(c.condition)).slice(0, 2);
    return {
...
function getRecommendedActions(conditions) {
...
export {
  ...
  Close as a,
  RiskBadge as b,
  getConfidenceTextClass as c,
  getConfidenceColorClass as d,
  getOverallImpression as e,
  getTopConditionsForSummary as f,
  getConditionDisplayName as g,
  getRecommendedActions as h,
  getConditionMeaning as i,
  getEvidenceStrength as j,
  RESEARCH_DISCLAIMER as k,
  Trigger as l,
  createDialogScope as m,
  parseRiskLevel as p,
  ue as u
};
